...
router.get('/', (req, res) => {
  ...
  if (!minDuration) return res.json(MOVIES); // si on trouve pas, alors on renvoie tous les films qu'on a
...
router.post('/', (req, res) => {
  ...
  const existingMovie = movies.find((film) => film.title.toLowerCase() === title.toLowerCase());
...
router.delete('/:id', (req, res) => {
  ...
  // eslint-disable-next-line no-use-before-define, eqeqeq
...
router.patch('/:id', (req, res) => {
  ...
  // eslint-disable-next-line eqeqeq
  const movieId = films.findIndex((film) => film.id == req.params.id);
  if (movieId < 0) return res.sendStatus(404);
